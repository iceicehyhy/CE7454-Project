...
#             nn.ReLU(True)
        ...
        self.attention_layer = ContextualAttention(ksize=3, stride=1, rate=2, fuse_k=3, softmax_scale=10,
    ...
    def forward(self, image, image_g1, mask):
        x1_inpaint = image_g1 * mask + image * (1. - mask)
        ones = torch.ones(image.size(0), 1, image.size(2), image.size(3))
        ones = ones.cuda()
        mask = mask.cuda()
        xnow = torch.cat([x1_inpaint, ones, mask], dim=1)
        encoder_x = self.encoder(xnow)
        encoder_x = self.middle(encoder_x)
        attention_x = self.attention_branch(xnow)
        attention_x, offset_flow = self.attention_layer(attention_x, attention_x, mask=None)
        #print (encoder_x.shape, attention_x.shape)
        x = torch.cat([encoder_x, attention_x], dim = 1)
        x = self.decoder(x)
        x = (torch.tanh(x) + 1) / 2
